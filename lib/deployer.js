...
module.exports = function(args) {
  ...
  var deployDir = pathFn.join(baseDir, '.basedir_git');
  var baseGitDir = pathFn.join(baseDir, '.git');
  var ignoreHidden = args.ignore_hidden;
  var ignorePattern = new RegExp(args.ignore_pattern);
  var log = this.log;
  ...
  return fs.exists(deployDir).then(function(exist) {
    ...
    var opts = {};
    log.info('Copying files from base folder...');
    opts.ignoreHidden = ignoreHidden;
    opts.ignorePattern = ignorePattern;

    return fs.copyDir(baseDir, deployDir, opts);
  }).then(function() {
    return parseConfig(args);
  }).each(function(repo) {
    return push(repo);
  }).then(function () {
    log.info("Remove base dir git");
    fs.rmdir(baseGitDir);
  });
};
